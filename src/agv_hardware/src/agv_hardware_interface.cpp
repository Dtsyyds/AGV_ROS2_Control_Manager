    ...
    const hardware_interface::HardwareComponentInterfaceParams & params)
{
    ...
    cartesian_path_sub_ = cmd_node_->create_subscription<geometry_msgs::msg::PoseArray>(
        ...
        [this](const geometry_msgs::msg::PoseArray::SharedPtr msg) {
            ...
            RCLCPP_INFO(cmd_node_->get_logger(), "收到路径规划轨迹，点数: %zu", msg->poses.size());
            ...
            for (const auto& pose_in : msg->poses) {
                ...
                try {
                    // 将点从 camera_link 转换到 base_link
                    // 允许等待 0.1s 让 TF 数据到位，解决 "extrapolation into the future" 错误
                    stamped_out = tf_buffer_->transform(stamped_in, "base_link", tf2::durationFromSec(0.1));
                    
                    // 将四元数转回欧拉角 (rx, ry, rz)
                    tf2::Quaternion q;
                    tf2::fromMsg(stamped_out.pose.orientation, q);
                    double rx, ry, rz;
                    ...
                    RCLCPP_ERROR(cmd_node_->get_logger(), "轨迹坐标转换失败: %s", ex.what());
                    return; // 转换失败则放弃整条轨迹
                }
            }
            ...
            if (!cartesian_path.empty()) {
                ...
                // 默认使用 linear 模式，速度 100.0 (根据之前代码习惯)
                robot_controller_->sendCartesianCommand(cartesian_path, "linear", 100.0);
            }
        });
